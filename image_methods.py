...
FITS_XPA_METHOD = "7f000001:59642"
...
print("\nMax number of files you will be able to process is:")
os.system("ulimit -n")
print("\n\n")
...
@utils.timeit
def detect_comet_frcnn(imarray, do_show=True, im_type="all_bands",
	...
	"""
	path_to_model = f"models/{model}/{im_type}/{classes_string}/tuned"\
					+ "/frozen_inference_graph.pb"
	path_to_labels = f"data/{im_type}/{classes_string}/train/labels.pbtxt"

	# normalize imarray:
	if isinstance(imarray, list) or imarray.ndim != 4:
		if isinstance(imarray, list):
			imarray = [resize_image(im, 512) for im in imarray]
	...
	labels = ["comet", "one_star", "defect", "planet"]
...
def get_region_masks(fits_file, region_file, mode="comet"):
	...
	# how regions are colored in the region file
	color_key = {
		"comet" : "green",
		"one_star" : "white", 
		"mul_star" : "red", 
	...
	if mode == "labels":
		label_key = dict((reversed(item) for item in color_key.items()))
...
def annotate_image(fits_file, reg_file=None):
	...
	with fits.open(fits_file) as f:
		...
		for key, val in color_key.items():
...
@utils.timeit
def annotate_composite(fits_files, reg_file=None, size=512):
	...
	# opened_files = [fits.open(f) for f in fits_files]
	try:
		dim=pyds9.DS9(FITS_XPA_METHOD)
	except:
		print("Looks like your SAO DS9 is not connected")
...
def set_region_type_to_image(reg_file, fits_file):
	with fits.open(fits_file) as f:
		...
		region_type = region_string.split("\n")[2]
...
@utils.timeit
def detect_comet(imarray, interactive=False, com_dir=None, non_dir=None, 
		space_dir=None, parallelize=True, do_show=False):
	"""
	...
	processed = resize_image(imarray, 512)
