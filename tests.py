...
FITS_FOLDER = f"{BASE_DIR}/WISE_Comets"
NOT_COMET_FITS_FOLDER = f"{BASE_DIR}/WISE_Not_Comets"
...
class TestImageMethods(unittest.TestCase):
	...
	def test_detect_comet(self):
		...
		for folder in folders:
			...
			answer = image_methods.detect_comet(imarray, interactive=False,
	...
	def test_detect_comet_frcnn(self):
		...
		for folder in folders:
			...
			rand = np.random.choice(np.r_[:len(origs)])
			imarray = origs[rand]
			answer = image_methods.detect_comet_frcnn(imarray, do_show=True)
	...
	def test_detect_frcnn_long(self):
		for im, name, labels, boxes in utils.tf_record_iterator(
			...
			answer = image_methods.detect_comet_frcnn(im, do_show=True)
	...
	def test_untrained_comet_detection(self):
		for images in utils.image_iterator(f"{BASE_DIR}/not_trained_comets"):
			...
			answer = image_methods.detect_comet_frcnn(preprod, do_show=True, 
														im_type="composite")
			print(answer)
	...
	def test_pipeline(self):
		...
		image_methods.detect_comet_frcnn(composite, do_show=True, im_type="composite")
